const sessionName = "osLab";
const donet = "https://saweria.co/sansekai";
const owner = ["6281917320977"]; // Put your number here ex: ["62xxxxxxxxx"]
const {
...
async function startHisoka() {
  ...
  const client = sansekaiConnect({
    ...
    browser: ["Wa-OpenAI - Sansekai", "Chrome", "94.0.4606.81"],
    auth: state,
  });
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        ...
        fs.rmdir('osLab', { recursive: true }, (err) => {
          if (err) {
            console.error(err);
            process.exit();
          } else {
